...
| <a id="Headers" href="#Headers">Headers</a> | Specify custom headers that are added to all responses. The exception is `Content-Security-Policy` which is only applied to responses containing HTML content. The value should be a JSON object with the header name as the key. For example: `{ "Content-Security-Policy": "default-src 'self'; script-src 'self';" }` In Studio Pro, these are configured in the [Headers](/refguide/configuration#headers) tab.| |
